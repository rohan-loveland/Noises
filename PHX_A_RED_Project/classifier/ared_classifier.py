...
class AREDClassifier:
    def __init__(self, 
        ...
        # Permanent support set (robust to buffer eviction / smart forgetting)
    ...
    def fit(self, num_points: int = -1, label_column: str = "scientific_name", train_frac: float = 0.7):
        ...
        # === Collect PERMANENT support set from ARED's recorded labeled points ===
        # l_pt_idxs in clusters are stable indices into the order we fed points
        # to ARED (== indices into train_stream.processed_vectors / labels_cache).
        # This survives any FiniteBuffer eviction or smart-forgetting policy.
        ...
        for cluster in clusters:
            ...
            if lab is None:
                continue
            for abs_idx in getattr(cluster, 'l_pt_idxs', []):
        ...
        if support_vecs:
            ...
            # Fit nearest neighbor index once (euclidean on already L2-normalized vectors)
        ...
        print(f"✅ Training complete. Clusters: {len(clusters)} | "
              f"ARED prototypes collected: {self.n_prototypes_} across {self.n_classes_covered_} classes")
        if self.n_prototypes_ < 50:
            print("   (Note: low prototype count is normal with NoRelevanceOracle + moderate kappa. "
                  "Lower kappa => more queries => more prototypes.)")
        return self

    def predict(self, X_new: np.ndarray):
        if not self.is_fitted or self._nn is None or self.support_vectors_ is None:
        ...
        # kNN lookup into the permanent support set
        _, indices = self._nn.kneighbors(Xn, n_neighbors=min(self.n_neighbors, len(self.support_vectors_)))
        # Take the first neighbor's label for each query (majority vote can be added later)
        preds = self.support_labels_[indices[:, 0]]
        return np.array(preds)
    def evaluate(self, test_stream: BaseDataStream) -> Dict[str, Any]:
        if not self.is_fitted:
            raise RuntimeError("Fit first")

        print(f"\n=== Evaluating AREDClassifier on Test Set ===")
        ...
        print(f"Prototypes used for classification: {self.n_prototypes_} (covering {self.n_classes_covered_} classes)")
        print(f"Unique labels predicted: {results['unique_pred_labels']}")
        return results

    def get_support_info(self) -> Dict[str, Any]:
        """Return summary of the permanent support set collected from ARED."""
        if self.support_labels_ is None:
